...
class AdaptiveBatAlgorithm(Algorithm):
	...
	def initPopulation(self, task):
		...
		A, S, Q, v = fullArray(self.A, self.NP), full([self.NP, task.D], 0.0), full(self.NP, 0.0), full([self.NP, task.D], 0.0)
	...
	def runIteration(self, task, Sol, Fitness, best, f_min, A, S, Q, v, **dparams):
		...
		for i in range(self.NP):
			...
			if self.rand() > self.r: S[i] = self.localSearch(best=best, A=A[i], task=task, i=i, Sol=Sol)
			Fnew = task.eval(S[i])
			if (Fnew <= Fitness[i]) and (self.rand() < A[i] / self.A): Sol[i], Fitness[i] = S[i], Fnew
			if Fnew <= f_min: best, f_min, A[i] = S[i], Fnew, self.updateLoudness(A[i])
		return Sol, Fitness, best, f_min, {'A': A, 'S': S, 'Q': Q, 'v': v}
...
class SelfAdaptiveBatAlgorithm(AdaptiveBatAlgorithm):
	...
	def runIteration(self, task, Sol, Fitness, best, f_min, A, r, S, Q, v, **dparams):
		r"""Core function of Bat Algorithm.
		...
		for i in range(self.NP):
			...
			v[i] += (Sol[i] - best) * Q[i]
			S[i] = task.repair(Sol[i] + v[i], rnd=self.Rand)
			if self.rand() > r[i]: S[i] = self.localSearch(best=best, A=A[i], task=task, i=i, Sol=Sol)
			Fnew = task.eval(S[i])
			if (Fnew <= Fitness[i]) and (self.rand() < (self.A_l - A[i]) / self.A): Sol[i], Fitness[i] = S[i], Fnew
			if Fnew <= f_min: best, f_min = S[i], Fnew
		return Sol, Fitness, {'A': A, 'r': r, 'S': S, 'Q': Q, 'v': v}
