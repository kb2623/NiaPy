...
class CoralReefsOptimization(Algorithm):
	...
	@staticmethod
	def algorithmInfo():
		r"""Get basic information of CoralReefsOptimization algorithm.
	...
	def typeParameters():
		...
		Returns:
			Dict[str, Callable]:
				* N (Callable[[int], bool])
				* phi (Callable[[float], bool])
				* Fa (Callable[[float], bool])
				* Fb (Callable[[float], bool])
				* Fd (Callable[[float], bool])
				* k (Callable[[Union[float, int], bool]):
		"""
		return {
			'N': lambda x: isinstance(x, int) and x >= 1,
			'phi': lambda x: isinstance(x, float) and x > 0,
			'Fa': lambda x: isinstance(x, float) and 0 <= x <= 1,
			'Fb': lambda x: isinstance(x, float) and 0 <= x <= 1,
			'Fd': lambda x: isinstance(x, float) and 0 <= x <= 1,
