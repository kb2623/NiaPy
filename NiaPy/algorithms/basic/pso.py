...
from NiaPy.util import fullArray, reflectRepair, Task
...
class ParticleSwarmAlgorithm(Algorithm):
	...
	"""
	Name: List[str] = ['WeightedVelocityClampingParticleSwarmAlgorithm', 'WVCPSO']
	...
	def setParameters(self, NP: int = 25, C1: float = 2.0, C2: float = 2.0, w: Union[float, np.ndarray] = 0.7, vMin: Union[float, np.ndarray] = -1.5, vMax: Union[float, np.ndarray] = 1.5, Repair: Callable[[np.ndarray, np.ndarray, np.ndarray, dict], np.ndarray] = reflectRepair, **ukwargs: dict) -> None:
		...
		Args:
			...
			Repair: Repair method for velocity.
	...
	def getParameters(self) -> Dict[str, Union[int, float, np.ndarray]]:
		...
		Returns:
			Dictionari which has parameters maped to values.
		...
		d.update({
			'C1': self.C1, 'C2': self.C2, 'w': self.w, 'vMin': self.vMin, 'vMax': self.vMax
		})
		return d

	def init(self, task: Task) -> Dict[str, Union[float, np.ndarray]]:
		...
		"""
		return {'w': fullArray(self.w, task.D), 'vMin': fullArray(self.vMin, task.D), 'vMax': fullArray(self.vMax, task.D), 'V': np.full([self.NP, task.D], 0.0)}
	...
	def updateVelocity(self, V: np.ndarray, p: np.ndarray, pb: np.ndarray, gb: np.ndarray, w: np.ndarray, vMin: np.ndarray, vMax: np.ndarray, task: Task, **kwargs: dict) -> np.ndarray:
		r"""Update particle velocity.

		Args:
			V: Current velocity of particle.
			p: Current position of particle.
			pb: Personal best position of particle.
			gb: Global best position of particle.
			w: Weights for velocity adjustment.
			vMin: Minimal velocity allowed.
			vMax: Maximal velocity allowed.
			task: Optimization task.
	...
	def runIteration(self, task: Task, pop: np.ndarray, fpop: np.ndarray, xb: np.ndarray, fxb: float, popb: np.ndarray, fpopb: np.ndarray, w: np.ndarray, vMin: np.ndarray, vMax: np.ndarray, V: np.ndarray, **dparams: dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float, dict]:
		...
		Args:
			...
			popb: Particles best position.
			fpopb: Particles best positions fitness/function values.
			w: Inertial weights.
			vMin: Minimal velocity.
			vMax: Maximal velocity.
...
class OppositionVelocityClampingParticleSwarmOptimization(ParticleSwarmAlgorithm):
	...
	def setParameters(self, p0: float = .3, w_min: Union[float, np.ndarray] = .4, w_max: Union[float, np.ndarray] = .9, sigma: Union[float, np.ndarray] = .1, C1: float = 1.49612, C2: float = 1.49612, **kwargs: dict) -> None:
		r"""Set core algorithm parameters.

		Args:
			p0: Probability of running Opposite learning.
			w_min: Minimal value of weights.
			w_max: Maximum value of weights.
			sigma: Velocity range factor.
			**kwargs: Additional arguments.
	...
	def oppositeLearning(self, S_l: np.ndarray, S_h: np.ndarray, pop: np.ndarray, fpop: np.ndarray, task: Task) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float]:
		r"""Run opposite learning phase.

		Args:
			S_l: Lower limit of opposite particles.
			S_h: Upper limit of opposite particles.
			pop: Current populations positions.
			fpop: Current populations functions/fitness values.
			task: Optimization task.
	...
	def runIteration(self, task: Task, pop: np.ndarray, fpop: np.ndarray, xb: np.ndarray, fxb: float, popb: np.ndarray, fpopb: np.ndarray, vMin: np.ndarray, vMax: np.ndarray, V: np.ndarray, S_l: np.ndarray, S_h: np.ndarray, **dparams: dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float, dict]:
		r"""Core function of Opposite-based Particle Swarm Optimization with velocity clamping algorithm.

		Args:
			task: Optimization task.
			pop: Current population.
			fpop: Current populations function/fitness values.
			xb: Current global best position.
			fxb: Current global best positions function/fitness value.
			popb: Personal best position.
			fpopb: Personal best positions function/fitness values.
			vMin: Minimal allowed velocity.
			vMax: Maximal allowed velocity.
			V: Populations velocity.
			S_l: Lower bound of opposite learning.
			S_h: Upper bound of opposite learning.
...
class CenterParticleSwarmOptimization(ParticleSwarmAlgorithm):
	...
	def setParameters(self, **kwargs: dict) -> None:
		r"""Set core algorithm parameters.
	...
	def runIteration(self, task: Task, pop: np.ndarray, fpop: np.ndarray, xb: np.ndarray, fxb: float, **dparams: dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float, dict]:
		...
		Args:
			task: Optimization task.
			pop: Current population of particles.
			fpop: Current particles function/fitness values.
			xb: Current global best particle.
			fxb: Current global best particles function/fitness value.
			**dparams: Additional arguments.
...
class MutatedParticleSwarmOptimization(ParticleSwarmAlgorithm):
	...
	def setParameters(self, nmutt: int = 10, **kwargs: dict) -> None:
		r"""Set core algorithm parameters.

		Args:
			nmutt: Number of mutations of global best particle.
			**kwargs: Additional arguments.
...
class MutatedCenterUnifiedParticleSwarmOptimization(MutatedCenterParticleSwarmOptimization):
	...
	def updateVelocity(self, V: np.ndarray, p: np.ndarray, pb: np.ndarray, gb: np.ndarray, w: np.ndarray, vMin: np.ndarray, vMax: np.ndarray, task: Task, **kwargs: dict) -> np.ndarray:
		...
		Args:
			...
			w: Weights for velocity adjustment.
			vMin: Minimal velocity allowed.
			vMax: Maxmimal velocity allowed.
			task: Optimization task.
			kwargs: Additional arguments.
...
class ComprehensiveLearningParticleSwarmOptimizer(ParticleSwarmAlgorithm):
	...
	def setParameters(self, m: int = 10, w0: float = .9, w1: float = .4, C: float = 1.49445, **ukwargs: dict) -> None:
		r"""Set Particle Swarm Algorithm main parameters.
	...
	def init(self, task: Task) -> Dict[str, np.ndarray]:
		r"""Initialize dynamic arguments of Particle Swarm Optimization algorithm.
	...
	def generatePbestCL(self, i: int, Pc: float, pbs: np.ndarray, fpbs: np.ndarray) -> np.ndarray:
		r"""Generate new personal best position for learning.

		Args:
			i: Current particle.
			Pc: Learning probability.
			pbs: Personal best positions for population.
			fpbs: Personal best positions function/fitness values for persolan best position.
	...
	def updateVelocityCL(self, V: np.ndarray, p: np.ndarray, pb: np.ndarray, w: np.ndarray, vMin: np.ndarray, vMax: np.ndarray, task: Task, **kwargs: dict) -> np.ndarray:
		r"""Update particle velocity.
	...
	def runIteration(self, task: Task, pop: np.ndarray, fpop: np.ndarray, xb: np.ndarray, fxb: float, popb: np.ndarray, fpopb: np.ndarray, vMin: np.ndarray, vMax: np.ndarray, V: np.ndarray, flag: np.ndarray, Pc: np.ndarray, **dparams: dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
		r"""Core function of algorithm.
