...
from NiaPy import benchmarks, util, algorithms, data
from NiaPy.algorithms import basic as balgos, modified as malgos, other as oalgos, Individual
...
NiaPyAlgos = [
	...
	balgos.ParticleSwarmOptimization,
	balgos.ParticleSwarmAlgorithm,
	balgos.OppositionVelocityClampingParticleSwarmOptimization,
	balgos.CenterParticleSwarmOptimization,
	balgos.MutatedParticleSwarmOptimization,
	balgos.MutatedCenterParticleSwarmOptimization,
	...
	balgos.BlackHole,
	balgos.ComprehensiveLearningParticleSwarmOptimizer,
	balgos.MutatedCenterUnifiedParticleSwarmOptimization
]
...
class Runner:
	...
	def getAlgorithm(name):
		...
		for alg in NiaPyAlgos:
			if name in alg.Name: return alg
	...
	def __exportToXls(self):
		...
		"""
		self.__createExportDir()
		workbook = None  # xlsxwriter.Workbook(self.__generateExportName('xlsx'))
		...
		for alg in self.useAlgorithms:
			...
			for bench in self.useBenchmarks:
				...
				for _ in range(self.nRuns):
					...
					res = algorithm.run(bm)
					self.results[alg][benchName].append((res[0] if not isinstance(res[0], Individual) else res[0].x, res[1]))
